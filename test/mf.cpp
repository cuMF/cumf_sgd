    ...
    vector<Block> &blocks)
{
    ...
    //initialize the pointer to each block.
                              ...
                              vector<mf_int> &omega_q)
{
    ...
    */
    ...
    /*
    string model_path = string("/home/xiaolong/programming/xx_libmf-2.01/08_movielens_baseline_test/test_input.model");
    FILE *fp;
    ...
    while ((read = getline(&line, &len, fp)) != -1) 
    {
        ...
        if(index < model->m)
        {
            for(int i = 0;i < model->k;i++)
            {
                model->P[index*model->k + i] = data[i];
            }
        }
        else
        {
            for(int i = 0;i < model->k;i++)
            {
                model->Q[(index - model->m)*model->k + i] = data[i];
            }
        }
...
protected:
    ...
    virtual float_pair sg_update(mf_int d_begin, mf_int d_end, __m128 XMMz,
                           __m128 XMMlambda_p1, __m128 XMMlambda_q1,
    ...
    while(!scheduler.is_terminated())
    {
        ...
        while(block->move_next())
        {
            ...
            float_pair ret = sg_update(0, 
                      ...
                      XMMeta, 
                      XMMrk_slow);
            p_total = p_total + ret.p;
            q_total = q_total + ret.q;
            count ++;
            //printf("p:%f, q%f\n", ret.p, ret.q);
            if(slow_only)
                continue;
        ...
        //printf("ave_p:%f, ave_q:%f\n", p_total/count, q_total/count);
...
protected:
#if defined USESSE
    float_pair sg_update(mf_int d_begin, mf_int d_end, __m128 XMMz,
                   __m128 XMMlambda_p1, __m128 XMMlambda_q1,
                   __m128 XMMlambda_p2, __m128 XMMlambda_q2,
...
#if defined USESSE
float_pair MFSolver::sg_update(mf_int d_begin, mf_int d_end, __m128 XMMz,
                                __m128 XMMlambda_p1, __m128 XMMlambda_q1,
                                __m128 XMMlambda_p2, __m128 XMMlambda_q2,
                                __m128 XMMeta, __m128 XMMrk)
{
    //printf("MFSolver::sg_update()\n");

    __m128 XMMpG = _mm_load1_ps(pG);
    __m128 XMMqG = _mm_load1_ps(qG);
    
    //__m128 XMMeta_p = XMMeta;
    //__m128 XMMeta_q = XMMeta;
    ...
    _mm_store_ps(p4, XMMeta_p);
    _mm_store_ps(q4, XMMeta_q);

    float_pair ret_v;

    ret_v.p = (p4[0] + p4[1] + p4[2] + p4[3])/4;
    ret_v.q = (q4[0] + q4[1] + q4[2] + q4[3])/4;
...
float_pair BPRSolver::sg_update(mf_int d_begin, mf_int d_end, __m128 XMMz,
                                 __m128 XMMlambda_p1, __m128 XMMlambda_q1,
                                 __m128 XMMlambda_p2, __m128 XMMlambda_q2,
                                 __m128 XMMeta, __m128 XMMrk)
{
    ...
    mf_double *cv_error)
{
    ...
    printf("lambda_p2:%.6f, lambda_q2:%.6f\n", param.lambda_p2, param.lambda_q2);
    if(!param.quiet)
    {
        cout.width(4);
        cout << "iter";
    ...
    //start to compute, most important codes.
    ...
    {
        solvers[i] = SolverFactory::get_solver(sched, 
                                               block_ptrs,
                                               PG.data(), 
                                               QG.data(),
                                               *model, 
    ...
    printf("perf: %.3f updates/second\n", tr->nnz*param.nr_iters/elapsed);


    //printf("just for flag\n");
...
{
    shared_ptr<mf_model> model;
    //printf("fpsg() is called\n");
try
{
    //Construct and initialize some data structures.
    ...
    //Generate map and perform "random_shuffle"
    //what does random_shuffle do? Make blocks fair? The function comes from the std.
    ...
    //What is omega_p, omega_q?
    omega_p = vector<mf_int>(tr->m, 0);
    omega_q = vector<mf_int>(tr->n, 0);

    //remap the value in the sparse matrix.
    ...
    //nr_bins is the length of one side
    ptrs = util.grid_problem(*tr, param.nr_bins, omega_p, omega_q, blocks);

    model = shared_ptr<mf_model>(
            Utility::init_model(param.fun,tr->m, tr->n, param.k, avg/scale, omega_p, omega_q),[] (mf_model *ptr) { mf_destroy_model(&ptr); });
...
bool check_parameter(mf_parameter param)
{
    ...
    //printf("check_parameter() is called\n");
    if(param.k < 1)
    {
...
mf_int mf_save_model(mf_model const *model, char const *path)
{
    ...
    fwrite(&(tmp_f), sizeof(int), 1, fptr);
    fwrite(&(tmp_m), sizeof(int), 1, fptr);
    fwrite(&(tmp_n), sizeof(int), 1, fptr);
    fwrite(&(tmp_k), sizeof(int), 1, fptr);
    fwrite(&(model->b), sizeof(float), 1, fptr);

    auto write = [&] (mf_float *ptr, int size)
    {
        for(int i = 0; i < size; i++)
        {
            mf_float *ptr1 = ptr + (long long)i*model->k;
            fwrite(ptr1, sizeof(float), model->k, fptr);

            if(i > 0 && i % 1000000 == 0)printf("progress: %%%.3f\n", (double)100.0*i/size);
        }
...
mf_model* mf_load_model(char const *path)
{
    ...
    auto read = [&] (mf_float *ptr, mf_int size)
    {
        ...
        { 
            mf_float *ptr1 = ptr + (mf_long)i*model->k;
            count = fread(ptr1,sizeof(float),model->k,fptr);
...
//#endif
    ...
    //FILE* err_ptr = NULL;
    //err_ptr = fopen("rmse.log","wb");
    
    for(mf_long i = 0; i < prob->nnz; i++)
    {
        mf_node &N = prob->R[i];
        mf_float e = N.r - mf_predict(model, N.u, N.v);
        //fwrite(&e, sizeof(float), 1, err_ptr);

        loss += e*e;
        
        if(i%10000000 == 0 && i > 0)printf("progress: %%%.3lf, est_RMSE: %.4lf\n", ((double)100.0)*i/prob->nnz, sqrt(loss/(i+1)));
    }
